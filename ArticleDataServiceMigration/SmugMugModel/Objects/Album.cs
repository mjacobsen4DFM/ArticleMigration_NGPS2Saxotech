...
namespace SmugMugModel
{
    public class Album : SmugMugObject
    {
        ...
        public string jsonString { get; set; } // the json string 11.19.2012
        /// <summary>
        ...
        public Album GetInfo(string SitePassword)
        {
            ...
            if (resp.stat == "ok")
            {
                ...
                myAlbum.jsonString = ch.GetJSONstring(); // 11.19.2012 Peter Bruni save original json String
                myAlbum.basic = basic;
                //Hack: The class has 2 properties with the same name - Comments. I've separated them in 2 different properties (HasComments and CommentsList)
        ...
        public bool Delete(long AlbumId)
        {
            ...
            var resp = ch.ExecuteMethod<SmugMugResponse>("smugmug.albums.delete", basic, "AlbumID", AlbumId);
            if (resp.stat == "ok")
                return true;
            else
            {
